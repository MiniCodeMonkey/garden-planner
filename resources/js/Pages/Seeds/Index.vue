...
import {DocumentMagnifyingGlassIcon, SparklesIcon, XMarkIcon} from '@heroicons/vue/24/outline'
...
function search(requestedFilters) {
    const newFilters = {
    ...
    router.visit('seeds', {data: newFilters});
}

function handleOptionChange($event, section, val) {
    const filter = filters.find(item => item.id === section);

    if (filter.multiple) {
        if ($event.target.checked) {
            search({
                [section]: [
                    ...props.currentFilters[section], val
                ]
            });
        } else {
...
const sortOptions = [
    ...
    {name: 'Most inventory', value: 'most'},
    {name: 'Least inventory', value: 'least'},
]
...
const filters = [
    {
        ...
        name: 'Category',
        multiple: true,
        options: props.filterOptions.category,
    },
    {
        id: 'sun',
        name: 'Sun',
        multiple: true,
        options: props.filterOptions.sun,
    },
    {
        id: 'greenhouse',
        name: 'Green house',
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <span v-if="currentFilters.search">"{{ currentFilters.search }}"</span> Seeds
        </template>
                                            ...
                                            <div class="space-y-6">
                                                <div v-for="(option, optionIdx) in section.options" :key="option.value"
                                                    ...
                                                    <input :id="`filter-mobile-${section.id}-${optionIdx}`"
                                                           ...
                                                           :checked="currentFilters[section.id] && currentFilters[section.id].includes(option.value)"
                                                           @change="handleOptionChange($event, section.id, option.value)"/>
                                ...
                                <MenuItems
                                    ...
                                    <div class="py-1">
                                        <MenuItem v-for="option in sortOptions" :key="option">
                                            <button @click="search({ sort: option.value })"
                                                    :class="[currentFilters.sort === option.value ? 'bg-gray-100' : '', 'w-full text-left block px-4 py-2 text-sm font-medium text-gray-900']">
                                                {{
                                                    option.name
                                                }}
                                            </button>
